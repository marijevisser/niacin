    ...
    if os.path.isfile(sev_dir):
        ...
        if just_names == 0:
            ...
            if n_txtfiles < 1 and verbose:
                ...
                start_search = re.compile(r'recording started at sample: (\d*)')
                gap_search = re.compile(r'gap detected. last saved sample: (\d*), new saved sample: (\d*)')
                hour_search = re.compile(r'-(\d)h')
    ...
    for file in file_list:
        ...
        with open(file['fullname'], 'rb') as sev:
            ...
            if stream_header.file_version > 0:
                stream_header.fs = np.power(2.,(np.float64(stream_header.rate) - 12)) * 25000000 / stream_header.decimate
            else:
    ...
    if not use_outside_headers:
        ...
        for var_name in header.stores.keys():
            ...
            if 'ts' in header.stores[var_name].keys():
                if len(header.stores[var_name].ts) >= 1:
...
if __name__ == '__main__':
    store = ['vidx','vers','Lig1','iSt1','S1tg','STrg','Lig2','iSt2','S2tg','S3tg','Lig3','iSt3','S4tg','Lig4','iSt4']
    data = read_block("D:\\CaseTank\\beta090424-4", store=store)
    #data = read_block("D:\CaseTank\phil_beta", headers=heads, evtype=['epocs'])
    for k in data.stores.keys():
        print(k, len(heads.stores[k].onset))
