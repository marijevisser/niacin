    ...
    if data.snips:
        ...
        for k in data.snips.keys():
            ...
            for j in range(len(ts)):
                ...
                if len(ts_ind) > 0:
                    if len(ts_ind) > 1:
                        min_diff = np.minimum(np.abs(time_ranges[1, ts_ind[0]]-time_ranges[1, ts_ind[1]]), np.abs(time_ranges[1, ts_ind[0]]-time_ranges[1, ts_ind[1]]))
            ...
            for kk in data.snips[k].keys():
                if kk in ['ts', 'data'] or not isinstance(data.snips[k][kk], np.ndarray):
                    continue
